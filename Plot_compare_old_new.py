...
def plot2(system_size5, system_size7, system_size9, system_size11, PATH5, PATH7,
 PATH9, PATH11, plot_range, PATH_RL5, PATH_RL7, PATH_RL9, PATH_RL11, MWPM, MCTS_Old_NN):
    ...
    ax.scatter(P_error5, P_success5,s=100, label='MCTS (d=5)', color='steelblue', marker='o')
    ax.scatter(P_error7, P_success7,s=100, label='MCTS (d=7)', color='green', marker='D')
    ax.scatter(P_error9, P_success9,s=100, label='MCTS (d=9)', color='orange', marker='s')
    ...
    plt.xlabel('$P_e$', fontsize=24)
    plt.ylabel('$P_s$', fontsize=24)
    plt.title('Jämförelse RL och MCTS guidat av RL ', fontsize=24)
    plt.tick_params(axis='both', labelsize=24)
    fig.set_figwidth(8)
    fig.set_figheight(5)
    #plt.savefig('Results'+'/Agent_Total_Result_Plot'+'.png')
