...
args = {'cpuct': np.sqrt(2), 'num_simulations':100, 'grid_shift': system_size//2, 'discount_factor':0.95, 'rollout_length':1, 'rollout_length':2}
Ns = {}
Nsa = {}
Qsa = {}
Wsa = {}
...
def step(action, syndrom):
    ...
    if add_opperator == 3 or add_opperator ==2:
        if qubit_matrix == 0:
            syndrom[0][row][col] = (syndrom[0][row][col]+1)%2
            syndrom[0][(row-1)%system_size][col] = (syndrom[1][(row-1)%system_size][col]+1)%2
        elif qubit_matrix == 1:
            syndrom[1][row][col] = (syndrom[1][row][col]+1)%2
...
def firstMCTS(state):
    ...
    return all_Qsa, all_actions, best_action

all_Qsa, all_actions, best_action  = firstMCTS(copy.deepcopy(toric_code.current_state))
...
step(best_action, toric_code.current_state)
